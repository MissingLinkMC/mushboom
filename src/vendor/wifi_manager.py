...
loadConfig();
    ...
    def wants_accesspoint(cls) -> bool:
        ...
        return (
            cls.wlan().status() != network.STAT_GOT_IP
        )  # Discard intermediate states and check for not connected/ok
    ...
    def setup_network(cls) -> bool:
        ...
        try:
            ...
            log.error(
                "Failed to load config file: {}. No known networks selected".format(e)
            )
            ...
            for aNetwork in available_networks:
                if aPreference["ssid"] == aNetwork["ssid"]:
                    connection_data = {
                        ...
                        "bssid": aNetwork[
                            "bssid"
                        ],  # NB: One day we might allow collection by exact BSSID
        ...
        for new_connection in candidates:
            log.info(
                "Attempting to connect to network {0}...".format(new_connection["ssid"])
            )
            # Micropython 1.9.3+ supports BSSID specification so let's use that
            if cls.connect_to(
                ssid=new_connection["ssid"],
                ...
                try:
                    ...
                    cls._notify_connection_change(
                        "connected", ssid=new_connection["ssid"], ip=ip
                    )
                except Exception as e:
        ...
        if not connected and candidates:
            try:
                ...
                cls._notify_connection_change(
                    "connection_failed", attempted_networks=failed_ssids
                )
            except Exception as e:
        ...
        try:
            ...
            cls.accesspoint().active(
                cls.wants_accesspoint()
            )  # It may be DEACTIVATED here
        except OSError as e:
